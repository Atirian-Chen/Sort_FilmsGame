...
| Primary Metric | Completed sessions / `experiment_exposed` sessions. Assigned sessions are diagnostic only. |
...
| Pre-launch Requirements | One real UI exposure maps to one variant; session-level stable assignment; predefined stopping rules and sample thresholds; no early winner claim from interim fluctuations. |
