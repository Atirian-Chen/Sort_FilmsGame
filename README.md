...
一个面向公开传播的 **Streamlit 影视偏好挑战页**：用户通过连续的 **1v1 二选一**，在几分钟内生成自己的电影审美榜单、分享海报和同题挑战链接。
...
6. 新增对局口令，同一候选和同一口令会得到一致的出场顺序，方便朋友接力挑战。
...
16. 导出新增 Markdown 和“挑战 JSON”格式。
17. 好友可以导入彼此的 JSON 榜单，自动对比 Top 5 重合和最大分歧。
18. 分享文案会附带对局口令，引导朋友用同一题目挑战。

### 公开发布版新增

- 首页改为“今日电影挑战 + 快速开始 + 示例结果卡”
- 支持 `?challenge=<id>` 挑战入口
- 支持 Supabase 保存挑战片单和匿名事件
- 首页公开展示已完成榜单数、今日挑战数、平均比较次数
- 支持 `?admin=<token>` 私密数据看板
- 结果页新增挑战海报、挑战链接复制、发布文案复制
- 内置首发挑战：
  - 豆瓣 Top 50 快排
  - 诺兰电影偏爱榜
  - 宫崎骏动画榜
...
如果没有配置 Supabase，应用仍然可以运行，只是公开统计、短挑战链接和后台看板会自动降级。
...
- 标题候选：
  ...
  - 3 分钟排出你的电影审美名片
  - 你最爱的电影，真的是你以为的那一部吗？
- 内容结构：截图结果海报 → 说玩法 → 放挑战链接 → 邀请晒 Top 3。

### 第 4-5 天：发短视频录屏

- 录 30 秒：打开挑战 → 连续二选一 → 生成海报 → 复制挑战链接。
- 重点展示“很快”“能发图”“朋友能同题挑战”。
