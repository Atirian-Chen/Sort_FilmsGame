...
- 输入豆瓣 ID，把自己的“已看电影”整理成私人总榜，并可按评分、标记年份筛选。
- 自定义一份片单，发给朋友整理同一批电影。
...
- **豆瓣已看**：输入豆瓣 ID 或“看过”页面链接，整理自己的已看电影总榜；支持按豆瓣评分、标记年份筛选后再开始排序。
- **浏览器书签导入**：当服务器直接读取豆瓣已看失败时，用户可以在自己浏览器里用书签工具读取已看列表、海报、评分和标记日期，生成跨设备可用的片单 ID。
...
- 豆瓣已看浏览器导入会保存到 `imported_movie_lists`，默认 7 天过期；其中包含标题、海报 URL、评分和标记日期，用于跨设备继续和筛选。
...
├── import_store.py                   # 豆瓣已看书签导入、评分/日期清洗、跨设备片单 ID
...
4. 打开自己的豆瓣电影“看过”页面。
5. 点击书签栏里的“导入我的豆瓣已看”。
6. 页面会读取当前浏览器能看到的已看列表、海报、评分和标记日期，生成一个 `db-` 开头的片单 ID。
7. 手机端或其他设备输入这个 ID，就能继续整理同一份片单。

读取或导入成功后，可以先按评分选择“全部”“4 星及以上”“只看 5 星”，也可以按豆瓣标记年份选择今年、去年或指定年份；旧版本导入的片单如果没有评分或日期信息，应用会提示重新读取。
...
> 独立开发并上线影视偏好排序 Web App，将复杂排序拆解为连续 1v1 取舍，支持内置片单、自定义片单、豆瓣 Top250、豆瓣已看评分/年份筛选、结果海报、二维码分享和匿名数据看板。

如果上线后拿到真实数据，可以补成量化版本：

> 首周获得 X 次访问、Y 份完成名单、Z% 完成率和 N 次分享链接复制；基于用户反馈迭代豆瓣已看导入、评分/年份筛选、本地自动保存、移动端取舍布局和结果页传播链路。
...
- 通过浏览器书签导入方案解决服务器抓取豆瓣已看列表易被安全页拦截的问题，并解析豆瓣评分和标记日期，支持跨设备片单 ID。
